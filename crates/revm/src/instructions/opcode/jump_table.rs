...
macro_rules! create_jump_table {
    ($spec_id:tt, $is_static:tt) => {{
        static INSTANCE: OnceCell<[InstructionFn; 256]> = OnceCell::new();
        INSTANCE.get_or_init(|| {
            let jumptable: [InstructionFn; 256] = [
...
pub fn opcode_jump_table<const IS_STATIC: bool, SPEC: Spec>() -> &'static [InstructionFn; 256] {
    match SPEC::SPEC_ID {
        SpecId::FRONTIER => {
            create_jump_table!(SPEC_ID_FRONTIER, IS_STATIC)
        }
        _ => {
            create_jump_table!(SPEC_ID_LATEST, IS_STATIC)
            /*
