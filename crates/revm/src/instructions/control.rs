use crate::{gas, Host, interpreter::Interpreter, Return, Spec, SpecId::{*, self}};
use primitive_types::U256;

pub fn jump(interpreter: &mut Interpreter, _host: &mut dyn Host) -> Return {
...
pub fn jumpi(interpreter: &mut Interpreter, _host: &mut dyn Host) -> Return {
...
pub fn jumpdest(interpreter: &mut Interpreter, _host: &mut dyn Host) -> Return {
    gas!(interpreter, gas::JUMPDEST);
    interpreter.add_next_gas_block(interpreter.program_counter() - 1)
}

pub fn pc(interpreter: &mut Interpreter, _host: &mut dyn Host) -> Return {
    // gas!(interpreter, gas::BASE);
    push!(interpreter, U256::from(interpreter.program_counter() - 1));
    Return::Continue
}

pub fn ret(interpreter: &mut Interpreter, _host: &mut dyn Host) -> Return {
...
pub fn revert(interpreter: &mut Interpreter, _host: &mut dyn Host) -> Return {
    // zero gas cost gas!(interpreter,gas::ZERO);
    pop!(interpreter, start, len);
    let len = as_usize_or_fail!(len, Return::OutOfGas);
    if len == 0 {
